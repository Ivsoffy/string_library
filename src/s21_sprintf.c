...
void paste_int(char *str_part, int *i, long long int len, long long int argument, flags *opt) {
  ...
  int copy_i = *i;
  while (count <= len - 1) {
    if ((opt->flag)&&(count == (len - 1)&&(str_part[copy_i + len - count - 2]=='.'))){
      opt->flag = 0;
      argument /= 10;     
    }else{
...
void* s21_trim(const char* src, const char* trim_chars) {
    ...
    if (src != NULL && trim_chars != NULL) {
        ...
        char* end_ptr = (char*)src + strlen(src);
        while (start_ptr != NULL && strchr(trim_chars, *start_ptr)) {
          start_ptr++;
        }
        while (end_ptr != NULL && strchr(trim_chars, *(end_ptr - 1))) {
          end_ptr--;
        }
        rv = (char*)malloc(end_ptr - start_ptr + 1);
        if (rv != NULL) {
            strncpy(rv, start_ptr, end_ptr - start_ptr);
...
void* s21_insert(const char *src, const char *str, size_t start_index) {
    ...
    if (src != NULL && str != NULL) {
        ...
        char *ptr = rv;
        if (rv) {
            strncpy(rv, src, start_index);
            ptr += start_index;
            strcpy(ptr, str);
            ptr += len_str;
            strcpy(ptr, src + start_index);
        }
    }
    return rv;
}
